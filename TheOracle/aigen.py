...
def s_init_lumaai():
  ...
  config = s_load_json("config.json")
  return LumaAI(auth_token=config["lumaai_key"])
...
def DEL_run_image(template_key, arguments_key): # DEPRECATED - Synchronous entry point
  client = s_init_openai()
  template = s_get_template(template_key)

  arguments = s_get_arguments(arguments_key)
  for arg_set in arguments: # from a logic standpoint, this is the producer, or emitter of events
    prompt = s_generate_prompt(template, arg_set) # and these functions are the consumer
...
# Runs the script using the synchronous entry point
#run_image("gothfembust", "sevenschools")
#run_image("dreamfembust", "sevenschools")

# Runs the script using the asynchronous entry point
asyncio.run(co_run_images("gothfembust", "sevenschools"))
#asyncio.run(co_run_images("dreamfembust", "sevenschools"))

# Generates a set of TTS from text.json
#run_tts()

# Generates a video segment (this script needs extensive work before being run again)
#run_luma()
